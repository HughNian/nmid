...
func (d *Dashboard) handler(w http.ResponseWriter, r *http.Request) {
	tmpl, err := template.ParseFiles("dashboard/template.html")
	if err != nil {
	...
	d.DiscoveryNum = dwfNum["discovery_num"].(int)
	d.WorkerNum = dwfNum["worker_num"].(int)
	d.FuncNum = dwfNum["func_num"].(int)
